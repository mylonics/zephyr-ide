...
export interface BuildDetails {
  ...
  /** Project-level runners (inherited by builds with the same name). */
  projectRunners: { name: string; config: RunnerConfig }[];
  /** Currently active build-level runner name. */
...
export function getBuildDetails(wsConfig: WorkspaceConfig, projectName: string, buildName: string): BuildDetails | undefined {
  ...
  const projectRunners: { name: string; config: RunnerConfig }[] = [];
  for (const [rName, rConfig] of Object.entries(project.runnerConfigs ?? {})) {
    projectRunners.push({ name: rName, config: rConfig });
  }

  const activeRunner = wsConfig.projectStates[projectName]?.buildStates[buildName]?.activeRunner;

  const buildFolder = getBuildFolder(wsConfig, project, build);
  const runnersYamlHint = getRunnersYamlHint(buildFolder);
