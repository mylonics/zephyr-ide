...
| `zephyr-ide.disableZephyrBaseInjection` | boolean | false | Disable Zephyr IDE's automatic injection of `ZEPHYR_BASE` into terminals, tasks, and build/debug processes. Enable this if you manage `ZEPHYR_BASE` yourself. |
| `zephyr-ide.zephyrBaseOverride` | string \| null | null | Absolute or workspace-root-relative path to a Zephyr base directory. When set, overrides the `ZEPHYR_BASE` value Zephyr IDE would otherwise inject into terminals, tasks, and build/debug processes. It does not change which Zephyr tree Zephyr IDE itself scans for boards, samples, DTS bindings, or Python requirements — those still follow the detected installation. Ignored when `zephyr-ide.disableZephyrBaseInjection` is true. |
...
| `zephyr-ide.compileCommandsMode` | enum | `all` | Controls how Zephyr IDE aggregates build `compile_commands.json` files into `.vscode/compile_commands.json`: `active` includes only the active build, `project` includes all builds in the active project, and `all` includes all projects and builds. Invalid values fall back to `all`. |
