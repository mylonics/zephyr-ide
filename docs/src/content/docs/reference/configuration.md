...
| `zephyr-ide.zephyrBaseOverride` | string \| null | null | Absolute or workspace-root-relative path to a Zephyr base directory. When set, overrides the `ZEPHYR_BASE` value Zephyr IDE would otherwise inject. Ignored when `zephyr-ide.disableZephyrBaseInjection` is true. |
