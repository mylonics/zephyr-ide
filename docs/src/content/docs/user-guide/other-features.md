...
Each debug target can be bound to a custom `launch.json` configuration, or left on the default (`auto`), which routes through the built-in `zephyr-ide` debugger and translates `runners.yaml` into a cortex-debug session automatically. See [Runner Configurations](building-debugging.md#runner-configurations-flash--debug) for the full bind model.
