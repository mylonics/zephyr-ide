...
class Session:
    ...
    def save(self, path: str, minimal: bool = True) -> Dict[str, Any]:
        ...
        if minimal:
            # Write only the symbols that changed during this session (differ
            # from original_values at load time).  write_min_config() compares
            # against Kconfig *default* values, which would pull in every
            # symbol that the existing .config already had set to a non-default
            # value — producing far more entries than the user actually changed.
            lines: List[str] = [
